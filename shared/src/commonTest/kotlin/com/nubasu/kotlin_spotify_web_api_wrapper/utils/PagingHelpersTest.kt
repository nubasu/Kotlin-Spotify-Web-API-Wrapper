...
class PagingHelpersTest {
    ...
    @Test
    fun collectAllItems_returnsFirstErrorWithoutFetchingNext() = runTest {
        val first: SpotifyApiResponse<TestPage> = SpotifyApiResponse(
            statusCode = 429,
        ...
        var called = false
        val response = PagingHelpers.collectAllItems<TestPage, Int>(
    ...
    @Test
    fun collectAllItems_aggregatesSuccessPages() = runTest {
        val first = SpotifyApiResponse(
            statusCode = 201,
            data = SpotifyResponseData.Success(
                TestPage(
                    items = listOf(1, 2),
        ...
        val response = PagingHelpers.collectAllItems(
            firstPageResponse = first,
            nextUrlSelector = { it.next },
            itemsSelector = { it.items },
    ...
    @Test
    fun collectAllItems_returnsError_whenLaterPageFails() = runTest {
        val first = SpotifyApiResponse(
            statusCode = 201,
            data = SpotifyResponseData.Success(
                TestPage(
    ...
    @Test
    fun collectAllItems_withFetcherOverload_usesInitialPagingOptions() = runTest {
